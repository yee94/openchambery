...
- 停止响应在收到确认或会话结束后及时恢复输入；五秒未确认时提示重试，避免停止按钮一直转圈。迟到的停止响应不会影响新一轮任务。
- 上下文压缩完成、尚未取得新用量时显示“等待更新”，不再把未知用量显示为零或继续显示压缩前的数据；桌面、移动端和迷你对话保持一致。
- 移动端向上浏览接近历史顶部时自动加载更早消息，加载期间显示进度，无需手动点击加载按钮。

### OpenCode 更新

- 从官方 npm 查询最新稳定 OpenCode 2 版本，并按所连接的主机隔离更新提示；启动、切回应用和重连后会检查更新。
- 更新检测失败保留失败状态；升级使用检测到的目标版本，并在重启后验证实际运行版本。
...
- 切换会话后保留已加载正文；切走时仍在加载的消息会缓存到原会话，切回时可直接复用。
- 自动刷新、重连和手动同步统一使用原生消息投影与逐消息合并，并发刷新共享请求，避免实时更新阻止其他消息刷新。
- 重连恢复不再先清空对话；刷新失败时保留旧正文，后续进入会话可重试。
- 修复长时间断线后历史分页无法补齐中间消息的问题，保留已加载内容并按原生游标补齐，维持消息顺序。
- 已缓存正文不再因其他消息尚未加载完整或后台刷新失败而被整页加载占位遮住。
...
- 修复自动压缩分割线错位：分割线按消息顺序显示在压缩与后续回复之间，不再落到整段续写之后。
- 自动压缩和等待续写期间保持当前回合开放，避免把压缩前单步结束误判为整轮异常结束，造成活动区反复收起、展开。
- 适配 OpenCode 2 消息元数据先于正文到达的同步过程：首次进入会话时保持加载占位，避免出现一排没有正文的模型标题。
- 移动端标题下的同步提示跟随正文就绪状态，正确处理加载失败、重试及会话切换。

### 移动端附件

- 照片/文件选择半屏面板仅在 Android 显示；iOS 和其他非 Android 界面直接使用系统文件选择器。
...
- 改进冷启动时的会话状态恢复；补全消息后会主动核对尚未发现的会话状态，减少工具运行期间状态缺失。
- 恢复与实时会话一致的全局会话目录信息，修正会话提及搜索中的标题，并改进工作状态面板在新会话发送和不同窗口尺寸下的显示。
- 后续回复出现后收起“重启后继续”提示，并从用户消息展示中隐藏内部会话引用上下文。

### 桌面

- 修正 Windows 桌面构建中 Git Bash tar 解压目录的盘符路径格式，避免 OpenCode CLI 准备阶段失败。
...
- 改进补全与命令面板的搜索、键盘操作和结果展示，支持更清晰的匹配高亮与引用内容管理。
- 优化长会话时间线、历史加载、滚动定位和运行状态呈现，改善消息恢复及工具结果展示。
- 扩展移动端输入体验，并完善多端共享的会话草稿、发送和同步处理。

### 提供商与插件

- 新增浏览器提供商配置与状态入口，并补充提供商额度凭据展示和登录流程。
- 改进插件运行状态、加载反馈与设置界面。

### OpenCode 集成

- 更新 OpenCode 2 客户端与 schema 依赖，完善运行时路由、会话元数据和升级提示处理。
...
- 服务重启后的恢复提示单独显示，不再和普通思考状态叠在一起；新的活动开始后恢复正常反馈。
- 压缩检查点改成和时间线一致的折叠行，运行中有状态提示，摘要默认收起。
- 队列中的消息可以编辑回输入框。空闲时新消息直接发送，不再被已有队列拦住；忙碌、重试或自动审查时仍会进队列。
- 输出速度按各步骤的生成时间计算 tok/s，不再用墙钟或工具耗时估算。
- 未读标记只在执行成功时记一次；正在查看的会话不会在完成瞬间被标成已读。

### 手机

- 键盘和原生输入框不再挡住旁路对话，以及选中文字后的操作条。
- 提问、权限和表单使用同一套卡片样式。
- 最近会话列表在按下预览时就准备好内容，取消或松手不会闪出空列表。

### OpenCode 与桌面

- 位置重载只等待官方确认，不再把无关目录的读取失败算进结果。
- 修正 Windows 安装包准备 OpenCode CLI 时，解压路径被当成远程地址的问题。
...
- 改进长会话历史加载、压缩记录和执行错误的展示，补齐重连后的消息恢复。
- 恢复技能命令与原生消息队列交互，并完善消息编辑、撤回和发送状态。

### OpenCode 2 与桌面

- 对接 OpenCode 2 共享后台服务，改进运行时启动、重载与版本识别。
- 完善 Agent 配置、权限与提供商目录的兼容处理。
...
## [1.19.22-beta.1] - 2026-09-23

### 对话

- 自己执行的终端命令（`/shell`）改成整行左对齐的终端结果样式，不再套用用户气泡，也不再显示模型/智能体标题。
- 命令输出直接完整展示，保留原始空格和换行，不再需要先点「显示输出」；复制输出的按钮和运行中/失败状态提示保留。

## [1.19.21] - 2026-09-23

汇总 1.19.21-beta.1 ~ 1.19.21-beta.3。

### 会话与项目排序

- 手动拖动会话后保留调整后的顺序；后续会话活动只提升对应会话，其余会话保持相对顺序，折叠列表中的会话也遵循同一规则。
- 手动调整项目顺序后，会话的新活动可继续将所属项目移到顶部；重复状态更新保留用户刚调整的顺序。

### 对话

- 粘贴带空格的文件路径会保持为完整文件引用，不再在空格处截断，之后也可以继续输入。
- 同一轮里前面的生成失败会在后面成功后隐藏；仍在的失败改成紧凑提示，不再是大段红字。
- 问题倒计时在自定义输入时只暂停一次；已经暂停后再打字不会反复进入「正在暂停」。
- 新消息列表里的 Mermaid 图重新按原来的方式绘制；D2 和 Infographic 代码块不再掉成空壳，仍显示为代码卡片。

### 附件

- 移动端助理和正文的回形针都先弹出半浮层，再选照片或文件。

## [1.19.21-beta.3] - 2026-09-22
...
- 关闭「自动托管待回答问题」后，问题卡片不再继续倒计时。
- 选择选项、输入内容或切换问题后，立即停止该问题的倒计时。
- 去掉问题卡片上含混的同步提示文案，倒计时区域只保留时间和进度条。

## [1.19.17] - 2026-09-17

汇总 1.19.17-beta.1 ~ 1.19.17-beta.3。

### 问题自动托管

- 新增问题自动托管能力：提问后默认 30 秒倒计时并展示全宽动态进度条，超时自动由模型自行判断并继续执行；支持随时暂停倒计时、立即托管或手动作答。
- 聊天设置中可配置「自动托管待回答问题」开关，合理归入自动化行为区；关闭或重连不丢弃状态。
- 覆盖主 Agent 及 Subagent 问题，子代理与工作树提问均按独立会话与目录倒计时。
- 纯净倒计时体验：问题卡片底部仅保留时间与进度条动画，无多余冗余提示。
- 权威状态由服务端 / 扩展宿主持有，关闭面板或刷新不会丢失倒计时与认领状态。

### 助理

- 助理对话中的图片支持点击预览：Markdown 图片与上传附件图片打开共享查看器，支持画廊切换与缩放。
- 绝对路径与 `file://` 图片经运行时接口流式加载后预览，解决跨端本地路径加载问题。
- 切换助理或离开对话页时自动关闭预览，避免预览遮罩残留影响全局快捷键。

### 定时任务

- 同一条定时任务在多个 OpenChamber 进程同时运行时，到点只会触发一次。

## [1.19.17-beta.3] - 2026-09-16

### 问题自动托管

- 新增问题自动托管：提问后默认 30 秒倒计时，超时由主机自动代答继续执行；可随时暂停、立即托管或手动回复。
- 设置中可开关「问题自动托管」；权威状态由服务端 / 扩展宿主持有，关闭面板或重连同端点不会丢暂停与进行中的认领。
- 子代理与工作树目录中的提问同样纳入托管与倒计时，会话目标在托管阻塞期间不会误推进。
- 上游未真正发出的失败可释放认领并允许人工重试；超时或不明确结果保留不确定状态，避免重复提交。
- Web、桌面与 VS Code 共用同一套快照与 tip 刷新；VS Code 在扩展宿主侧计时，不依赖 webview 是否打开。

## [1.19.17-beta.2] - 2026-09-15

### 助理

- 助理对话中的图片支持点击预览：Markdown 图片与上传附件图片打开与常规会话一致的共享查看器，支持画廊切换与缩放。
- 绝对路径与 `file://` 图片经运行时接口流式加载后预览，不再因浏览器无法直接读取本地路径而失效。
- 切换助理或离开对话页时自动关闭预览，避免预览遮罩残留影响全局快捷键。
...
- 支持回查自动裁剪的聊天记忆：按关键词和时间范围搜索，分段读取原文，并根据消息来源自然回复。
- 记忆回查限定当前助理，遵守清除记忆和删除聊天记录的边界，服务重启后继续保留隔离状态。
- 修复排队清除记忆时，上一轮晚到回复仍进入上下文的问题，同时保留后续排队的新请求。
- 历史搜索和长消息读取采用有界分页，明确区分部分结果、查无记录和读取失败。

## [1.19.15] - 2026-09-12

汇总 1.19.15-beta.1 ~ 1.19.15-beta.5。

### 助理

- 执行前说明和阶段消息在模型生成过程中即时显示并保存，支持执行期间跨端恢复，完成时按消息 ID 去重。
- 保存默认提示词、清除记忆等操作由所选模型根据真实结果自然回复，不再出现固定确认文案；工具结果作为内部依据。
- 完成通知仅显示最新一条回复；模型调用异常以独立错误提示呈现，不再混入对话气泡。
- 助理设置支持选择、保存和恢复模型变体，执行与结果总结沿用所选推理配置。
- “全部已读”仅在有未读消息时显示，清零后收起整行；处理期间保持防重复点击。
- 多次工具调用和重试后，分段回答按原顺序完整保存；清除记忆后显示唯一确认，并同步清理当前页面的旧气泡。
- 停止会话时正确识别中断来源，避免额外回复；用户中断后停止自动续做。
- 回复气泡完成后立即跨端同步；执行中的等待状态持续跟随上游进度。
- 优先查找已有工作区并协调工作会话，简单查询和小范围操作可直接处理。
- 修复输入框多行高度和会话引用插入后的布局。
...
- 需要重装原生壳时，更新卡片也会显示 changelog，不再只在应用内 OTA 时才有更新内容。

## [1.19.15-beta.5] - 2026-09-12

### 助理

- 执行前说明和阶段消息在模型生成过程中即时显示并保存，支持执行期间跨端恢复，完成时按消息 ID 去重。
- 保存默认提示词、清除记忆等操作由所选模型根据真实结果自然回复，工具确认文案保留在内部上下文。
- 完成通知仅显示最新一条回复；模型调用异常以独立错误提示呈现。
- 助理设置支持选择、保存和恢复模型变体，执行与结果总结沿用所选推理配置。
...
汇总 1.19.13-beta.5 ~ 1.19.13-beta.9。

### 助手

- 对话保留近期历史和会话卡片上下文；新增插话、停止、归档、删除及只读会话内容查询，输入框提供真正的停止按钮。
- 修复读取技能后提前结束、伪造工具结果和错误报告完成的问题；中断会取消执行。
- 模型选择可识别当前实例的可用模型、收藏和最近使用；`@会话` 引用可读取被引用对话内容。
- 助手按最新消息排序；跨端同步未读和全部已读，iOS 原生标签栏显示未读角标。
- 发送请求等待期间仍可停止正在执行的任务。

### 界面

- 助理与新建对话图标更圆润；历史消息导航浮层更紧凑。
- 侧栏项目、会话分组与文件夹增加展开收起动画，支持减少动态效果设置。
...
- 会话引用弹层统一内边距，收紧列表和辅助文字排版。
- 底部助理标签显示总未读气泡，采用随主题切换的高对比配色，并与图标留出间距。
- 引用会话读取结果保留在模型工具上下文，聊天气泡展示助理的口语回复。
- 清空历史或开始新一轮对话后，清理过期的失败与超时提示。

### 移动端

- 修复模型选择器搜索框的键盘焦点冲突，改善 iOS 搜索时唤起键盘的稳定性。

## [1.19.13-beta.6] - 2026-09-11

### 界面

- 助理与新建对话图标采用更圆润的轮廓，新建对话图标更小巧。
- 历史消息导航浮层缩小字号并收紧行距，长消息预览更精致。
- 侧栏项目、会话分组与文件夹增加展开收起动画，支持减少动态效果设置，并同步更新虚拟列表布局。

### 助理

- 发送请求等待期间仍可停止正在执行的任务，改善停止按钮的响应。

## [1.19.13-beta.5] - 2026-09-11

### 助手

- 修复读取技能后提前结束、伪造工具结果和错误报告完成的问题；中断会取消执行，后台结果通知不再擅自重启任务。
- 对话保留近期历史和会话卡片上下文；新增插话、停止、归档、删除及只读会话内容查询，输入框提供真正的停止按钮。
- 模型选择可识别当前实例的可用模型、收藏和最近使用；模糊名称存在歧义时不再猜测模型 ID。
- `@会话` 引用复用普通聊天的 chip、编辑和传输格式，可读取被引用对话内容。
- 助手按最新消息排序；跨端同步未读和全部已读，iOS 原生标签栏显示未读角标。
- 回复语言遵循界面语言；修复长 Agent 名称导致消息头高度跳变的问题。

## [1.19.13] - 2026-09-11

汇总 1.19.13-beta.1 ~ 1.19.13-beta.4。

### 助手

- 首页列表描述改为最新消息两行预览；桌面与移动端补创建入口；设置页编辑不再误清 `agent` / `variant` / `mode`。
- 助手可读写自身设定里的默认提示词；继续已有编码会话时跟随该会话上一轮模型，目录没有则降级为助手默认。
- 「清空聊天记录」模型调用即执行；「清除记忆」只推进上下文水位并保留记录。编码任务结束后不再弹出「会话已完成 / 会话失败」，结果交回联系人助手总结或继续推进。
- 派单成功不再把英文工具确认写进对话；联系人附件持久化与发送路径加固。
- `assign_session` 副作用超时、显式模型 fail-closed、同轮去重；联系人回合乐观 UI 与当前回合附件随指派转发。

### 通知

- iOS Live Activity 后台按权威会话状态刷新；定时任务成功/失败通知全通道；通知开关补全「权限请求」「定时任务」「助手消息」「目标结算」。

### 会话 / Git

- 流式代码块 Shiki 高亮更稳定；变更树文件行只保留文件名。

### 移动端

- 从任意页面打开设置后，返回键回到原来的页面与次级面板。
- 切回设置标签页时恢复为一级列表。

### 运行时 / 中继

- 运行时认证与切换、passkey、relay 图片流与隧道客户端行为加固。
...
- 首页列表每项描述改为最新消息两行预览（参考微信）：服务端 snapshot 权威 `latestMessagePreview`，桌面与移动端共用两行省略；图片/文件/卡片用本地化标签，无消息显示空态，不拿默认提示词顶替。
- 桌面侧栏与移动端列表非空时补创建入口；设置页编辑保存不再误清 `agent` / `variant` / `mode`。
- 「清空聊天记录」模型调用即执行，不再做用户文本授权拦截；「清除记忆」仍只推进上下文水位并保留聊天记录。工具 wipe 按当前用户 ordinal 有界删除，保留后续已入队用户消息与去重身份，并清掉前序轮迟写的回复/卡片。
- 联系人附件：服务端持久化与上传下载、composer 附件缓存与发送路径加固；附件卡片渲染与 contact 消息分页/恢复测试补齐。

### 运行时 / 中继

- 运行时认证与切换、passkey、relay 图片流与隧道客户端行为加固。

## [1.19.13-beta.1] - 2026-09-09

### 通知

- iOS Live Activity 后台刷新：App 被挂起或杀掉后，服务器每 30 秒按权威会话状态重算 Live Activity 快照，仅在内容实际变化时推送更新（busy/retry 状态变化、新出现的可见会话、遗漏的完成、stale 恢复）；无变化不推送，节省 APNs 每小时更新预算。`OPENCHAMBER_LIVE_ACTIVITY_REFRESH_INTERVAL_MS` 可调，`0` 关闭。
- 定时任务通知：计划触发的任务运行在成功或失败时通知全通道（桌面 / 界面 / Web Push / APNs）；手动「立即运行」不通知。APNs 使用本地化场景标题（10 语言），失败详情只进桌面与 Web Push，不经过推送中继。
- 通知开关补全：新增「权限请求」「定时任务」「助手消息」「目标结算」四个独立开关（默认开启，设置 → 通知 → 事件）。助手消息不再跟随「任务完成」开关，权限通知不再跟随「问题」开关。

### 助手

- `assign_session` 健壮性：所有 worker 会话副作用加有限超时；可选字段验证 fail-closed（空白串/对象直接报错，不再静默回退）；新增 `model_ambiguous` / `prompt_ambiguous` 等错误码；同一联系人回合内重复指派去重复用既有会话，模型歧义时的部分成功只复用身份不重复建会话。
- 联系人回合乐观 UI：发送后立即上屏，权威回合状态到达后收敛；当前回合用户附件由服务器持有并默认随指派转发。
- 提供者/模型目录限载，挂起的 SDK 调用不再阻塞联系人通道；目录行经净化后供提示词与工具消费（无密钥）。

### 移动端

- 从任意页面打开设置后，返回键回到原来的页面与次级面板，而不是留在设置页。
- 切回设置标签页时恢复为一级列表，不再停留在上次的深层内页。
...
- 清除记忆的水位按当前轮用户消息截断，不会吞掉排队中已 admit 的后续用户消息。
- 联系人助手能在工作目录里读文件、改文件、跑命令，并保留建会话、建助理、定时任务等操作工具。
...
- 切换远程实例前会探测认证状态；远程更新入口仅对支持的非 Local 实例显示。
...
汇总 1.19.8-beta.1 ~ 1.19.8-beta.8。

### 助手

- 联系人助手能在工作目录里直接读文件、改文件、跑命令；工作目录、提示词和项目技能会一起带进 pi。
- 联系人对话跟着新消息走，上滑阅读时保持位置；超长路径会折行。

### 会话

- Markstream 正文改回 compact 代码卡片，切换会话后内容正常，样式不再串到旧卡片。
- 流式更新时保持阅读位置，轻微上滑后不会被拉回底部。

### 移动端

- 历史消息改由顶部「加载更早消息」按钮加载。
- 输入框按真实纵向手势展开收起，流式输出时不再闪动。

## [1.19.8-beta.8] - 2026-09-07

汇总 1.19.8-beta.1 ~ 1.19.8-beta.7。
...
- 修复正文轻微上滑后被流式更新拉回底部：累计识别小幅触摸，保留底部附近的阅读位置，向下回到底部后恢复跟随。
- 移动端代码卡、结果卡和正文预览保持裁剪，减少嵌套纵向滚动对阅读手势的干扰；保留代码横滑、换行与展开。

### 助手

- 联系人对话统一滚动锚定与边界策略，改善异步内容增高时的阅读稳定性。
...
- 历史消息统一由顶部「加载更早消息」按钮加载，滚动、上滑手势和短页面保持手动加载模式。
- 输入框展开与收起跟随真实纵向手势；流式输出、布局变化和历史补载期间保持当前形态，修复输入框闪动。

### 助手

- 联系人对话在底部跟随新消息及异步内容增高；上滑阅读时保持位置，手动回到底部后恢复跟随。
...
- 首次开启助手时自动种入名为「默认助理」的联系人。
- 联系人对话按短气泡往外说，过程中可以先回一句人话（思考不进对话）；能从已注册项目里开新会话，右上角可进该助手设置和定时任务。
- 绿点只出现在左侧助手列表，并且只在这条对话正在处理时亮；干活时三点会一直跟在消息下面。
- 助理发消息按联系人通知：标题是昵称，正文是原话，点开进该对话；只推导航栏里已经显示的会话，subagent 完成不再推。
- 助理和同伴气泡支持 Markdown；列表头像统一圆形，不再显示 provider/model 小胶囊。
- 手机对话正文补上顶部导航栏高度的留白；设置里只保留模型选择。

### 会话

- 助手正文（含流式）默认改用 Markstream React 渲染；列表缩进与代码字号对齐到切换前节奏。真实文件路径可点开预览，图片限制在气泡宽度内。要回到 marked + Shiki：`localStorage.setItem('oc:markstream-react','0')` 后刷新。
- 新建对话建立中时底部不再露出项目 / 分支芯片；草稿与正式 transcript 共用视口，助手模型头从 pending 起就占位。
- 还没有具体工具或正文时，模型头下方继续显示思考动画，不再单独亮底部「正在思考」状态条。
- 浅色主题下 live 文本 shimmer 更清晰；思考扫光与子代理忙碌文案用同一套 token。
- 正在发送时也可以按两下 Esc 中止；焦点在正文、展开输入或侧栏编辑器时不再被吃掉。
- 电脑端侧栏聚焦当前会话时平滑滚到视口大约三分之一处；展开「运行了」后的工具列表行距更松。

### 性能

- 关闭「显示推理轨迹」后，服务端发送前剔除推理正文与增量；关闭期间跳过推理缓存与补全，重新开启后恢复完整内容。VS Code 长时间思考期间保留轻量心跳。
- 关闭的时间线 / 模型选择面板不再构建列表；非当前显示的会话暂停流式渲染订阅。
- 会话列表行更新隔离；诊断记录批量写入，长会话滚动的卡顿与内存压力更低。

### 移动端

- 顶部通知改用和 Dock 一样的半透明毛玻璃；折叠的多层通知用表面明度差和错位露边区分。
- `/` `@` `#` 补全层 portal 到页面根节点，高度跟随键盘，不再被 IME 压到屏幕下沿。
- 默认主题改为 carbonfox-light / vitesse-dark，启动闪屏与状态栏底色对齐。
- iOS Live Activity 只显示导航栏里的主会话；后台完成后改成完成并停表。推送标题改成短句（任务已完成 / 需要你回答等），正文用会话名。
...
- 时间线 / 模型选择等面板关闭后不再构建列表内容，重开立即用最新数据，退出动画期间内容完整保留。
- 关闭面板期间不再重复订阅会话消息，多个会话并行推理时的手机 CPU 占用下降。
- 会话列表行更新隔离：某一行状态变化不再重建整棵首页列表；会话状态浮条关闭时只更新角标。
- 非当前显示的会话页面暂停流式渲染订阅与卡住自检，恢复显示时从权威数据续上，避免后台会话拖累主界面。
- 诊断记录改为批量写入并复用连接；会话数据重复投影大幅减少，长会话滚动的卡顿与内存压力更低。
...
- 联系人对话会按短气泡往外说，过程中可以先回一句人话（比如「我去找一下」），思考不会进对话。
- 能从已注册项目里找到仓库并开新会话；对话右上角可以进该助手设置和它排的定时任务。
- 绿点只出现在左侧助手列表，并且只在这条对话正在处理时亮。
- 每一轮对话结束后，会发短信样式的完成通知：标题是助手名，正文是这轮说的话。
- 输入框多行会按内容自动长高。

## [1.19.7-beta.13] - 2026-09-06

### 移动端

- 顶部通知改为更轻的原生横幅：去掉厚重描边和大阴影，保留克制的悬浮层次；状态图标统一为线性样式，按钮收成轻量文字操作。
- 折叠的多个通知通过逐层表面明度差与错位露边区分，深浅主题都能一眼看清又保持干净。
