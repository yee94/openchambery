...
- **MERGED** as PR #197 — tip advanced to `ea6bc02b26c8365492be8033c2e8040981d693cb` / published APK `lynx-v2-debug-ea6bc02` (see Next #156).
...
## Notes — Docs tip APK/SHA honesty post-#155 (Next #156 / 2026-09-22)

- Cap skim pending / **NO_NEW** expected from prior skim + docs-only tip delta. Settings search LynxInput stays 代码接上 (Next #139), not 三关 landed / not 真机过. Incremental leftovers unchanged: Scheduled history first page 代码接上; session-row badges / Files image/markdown polish/host; deferred `@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk.
- Tip / prerelease APK: `lynx-v2-debug-ea6bc02` (work/lynx-native @ `ea6bc02b26c8365492be8033c2e8040981d693cb`; PR #197 / Next #155 MERGED; release exists; asset `openchamber-lynx-debug-ea6bc02.apk`; published ~2026-09-22T04:35:17Z). Was stale `lynx-v2-debug-aca0d60` / tip `aca0d600069f792d86bd56fe937ef7442870a7f1`. Tip delta `aca0d60`→`ea6bc02` is docs-only. Lynx Mobile CI SUCCESS on tip `ea6bc02` (run 35687023213). Do not invent a newer APK SHA than the published tag.
- **MERGED** as PR #198 — tip advanced to `4d4ff1003faa2113760b25927404a09aff6045c5` / published APK `lynx-v2-debug-4d4ff10` (see Next #157).
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-ea6bc02`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).

## Notes — Docs tip APK/SHA honesty post-#156 (Next #157 / 2026-09-22)

- Cap skim pending / **NO_NEW** expected from prior skim + docs-only tip delta. Settings search LynxInput stays 代码接上 (Next #139), not 三关 landed / not 真机过. Incremental leftovers unchanged: Scheduled history first page 代码接上; session-row badges / Files image/markdown polish/host; deferred `@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk.
- Tip / prerelease APK: `lynx-v2-debug-4d4ff10` (work/lynx-native @ `4d4ff1003faa2113760b25927404a09aff6045c5`; PR #198 / Next #156 MERGED; release exists; asset `openchamber-lynx-debug-4d4ff10.apk`; published ~2026-09-22T07:41:51Z). Was stale `lynx-v2-debug-ea6bc02` / tip `ea6bc02b26c8365492be8033c2e8040981d693cb`. Tip delta `ea6bc02`→`4d4ff10` is docs-only. Lynx Mobile CI SUCCESS on tip `4d4ff10` (run 35700374946). Do not invent a newer APK SHA than the published tag.
- **MERGED** as PR #199 — tip `ceb8c30a9da2f97fc9360b189f2fd6ef007397b0` / published APK still `lynx-v2-debug-4d4ff10` at that merge; product tip later `841d2e71644a6513106f5b10091f280c065bac11` / published APK `lynx-v2-debug-841d2e7` (see Next #158 / Next #159), then `91c81f4e53167cafdebdd0e3ba23829ccb8101c0` / published APK `lynx-v2-debug-91c81f4` (Next #160 **MERGED** as PR #202 / Next #161 **MERGED** as PR #203), then `618ae12a5450a633f716cc95fb4e24dbe9b9844b` / published APK `lynx-v2-debug-618ae12` (Next #162 **MERGED** as PR #204), then tip `611d5433b2b6529243115b7dfdf111a77110c8f9` / published APK `lynx-v2-debug-611d543` (Next #163 **MERGED** as PR #205), then tip `3d0e7c314192401061db279c431c4ba66275ce1a` / published APK `lynx-v2-debug-3d0e7c3` (Next #164 **MERGED** as PR #206), then tip `c73a66e57e6040d7c7e86001edc0e421f03ff1de` / published APK `lynx-v2-debug-c73a66e` (Next #165 **MERGED** as PR #208), then tip `ac78413d08fef1d09b5baf4c4e054d952a242d96` / published APK `lynx-v2-debug-ac78413` (Next #166).
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-4d4ff10`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).

## Notes — Cap Assistant contact transcript (Next #158 / 2026-09-22)

- Cap phone Assistant conversation is the **contact transcript** (`GET /api/openchamber/assistants/:id/messages` stitched history + admit send + `session/abort` + Cap `/api/global/event` SSE), not project session ChatScreen. Lynx: `LynxContactConversationScreen` + `contactMessages` / `contactMerge` / `contactDisplay`; AssistantTab open → contact secondary; soft-ensure inside screen (no invented session id). Admit POST body matches Cap (`sessionID` / `sessionGeneration` / `messageID` / `parts`).
- Vitest: parse/merge/send-failure + list helpers. LegendList 1.19 load-older button only (no TanStack Virtual / no bounce fetch).
- Tip / APK honesty: write-time base tip `work/lynx-native` @ `ceb8c30a9da2f97fc9360b189f2fd6ef007397b0` (PR #199 / Next #157 MERGED). Published APK at write time: `lynx-v2-debug-4d4ff10` (do **not** invent `lynx-v2-debug-ceb8c30`). **MERGED** as PR #200 — later tip was `841d2e71644a6513106f5b10091f280c065bac11` / published APK `lynx-v2-debug-841d2e7` (Next #159 **MERGED** as PR #201). Current tip `ac78413d08fef1d09b5baf4c4e054d952a242d96` / published APK `lynx-v2-debug-ac78413` (Next #165 **MERGED** as PR #208; see Next #166). Product **NOT DONE** / 三关未齐 / not EXHAUSTED. No 真机过 claim.
- Residual at write time included "no contact-* SSE". That residual is struck by Next #160. Still deferred: host attachment picker/camera; Cap IntersectionObserver read-mark geometry; no invented `contact/cards` or `contact/dm` REST routes; Cap toast / `@dnd-kit` / iPad / host-only unchanged.

## Notes — Docs tip APK/SHA honesty post-#158 (Next #159 / 2026-09-22) — **MERGED** as PR #201

- Cap skim pending / **NO_NEW** expected from prior post-#158 residual list + this docs-only hop. The post-#158 "no contact-* SSE" residual is struck by Next #160 (same tip / published APK; do not invent a newer APK). Next #158 Cap Assistant contact transcript stays 代码接上 (`LynxContactConversationScreen`), not 三关 landed / not 真机过. Settings search LynxInput stays 代码接上 (Next #139). Incremental leftovers unchanged: Scheduled history first page 代码接上; session-row badges / Files image/markdown polish/host; deferred `@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk.
- Tip / prerelease APK: `lynx-v2-debug-841d2e7` (work/lynx-native @ `841d2e71644a6513106f5b10091f280c065bac11`; PR #200 / Next #158 MERGED; release exists; asset `openchamber-lynx-debug-841d2e7.apk`; published ~2026-09-22T10:55:11Z). Prior tip after #157 was `ceb8c30a9da2f97fc9360b189f2fd6ef007397b0`. Was stale published cite `lynx-v2-debug-4d4ff10`. Tip delta `ceb8c30`→`841d2e7` is product Cap Assistant contact transcript (PR #200), then Mobile CI published this APK. Lynx Mobile CI SUCCESS on tip `841d2e7` (run 35718022126). Do not invent a newer APK SHA than the published tag.
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged at write: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-841d2e7`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).
- **MERGED** as PR #201 — tip stayed `841d2e71644a6513106f5b10091f280c065bac11` / published APK `lynx-v2-debug-841d2e7` at that merge; product tip later `91c81f4e53167cafdebdd0e3ba23829ccb8101c0` / published APK `lynx-v2-debug-91c81f4` (see Next #160 / Next #161 **MERGED** as PR #203), then `618ae12a5450a633f716cc95fb4e24dbe9b9844b` / published APK `lynx-v2-debug-618ae12` (Next #162 **MERGED** as PR #204), then tip `611d5433b2b6529243115b7dfdf111a77110c8f9` / published APK `lynx-v2-debug-611d543` (Next #163 **MERGED** as PR #205), then tip `3d0e7c314192401061db279c431c4ba66275ce1a` / published APK `lynx-v2-debug-3d0e7c3` (Next #164 **MERGED** as PR #206), then tip `c73a66e57e6040d7c7e86001edc0e421f03ff1de` / published APK `lynx-v2-debug-c73a66e` (Next #165 **MERGED** as PR #208), then tip `ac78413d08fef1d09b5baf4c4e054d952a242d96` / published APK `lynx-v2-debug-ac78413` (Next #166).

## Notes — Cap contact-* SSE live tail (Next #160 / 2026-09-22) — **MERGED** as PR #202

- Lynx contact secondary parses real Cap envelopes `openchamber:contact-turn-start` / `contact-bubble-delta` / `contact-turn-end` and subscribes on existing `GET /api/openchamber/events` with the same runtimeFetch SSE opener chat uses for `/api/global/event`. Filter is `assistantID`. Malformed envelopes are dropped (not a fake turn). Optimistic bubbles land on the contact transcript.
- Optimistic composer rows move sending → admitted → failed. Bubble deltas become preview rows in the same LegendList as stitched history. Reconcile drops overlays the authoritative history page already covers. A failed history refresh keeps the previous pages (failure ≠ empty transcript). Admit and session abort stay wired; abort settles working previews without claiming success.
- Vitest: `packages/lynx/src/assistants/contactEvents.test.ts`, `contactOptimistic.test.ts`.
- Tip / APK honesty: write-time base `work/lynx-native` @ `841d2e71644a6513106f5b10091f280c065bac11` (PR #200 / Next #158 MERGED). Published APK at write `lynx-v2-debug-841d2e7` (prerelease; exists; asset `openchamber-lynx-debug-841d2e7.apk`; published ~2026-09-22T10:55:11Z). Lynx Mobile CI SUCCESS on that tip `841d2e7` (run 35718022126). Next #159 **MERGED** as PR #201 (docs honesty). Do **not** invent a newer APK than `lynx-v2-debug-841d2e7` as that write-time tag.
- **MERGED** as PR #202 — tip after merge `work/lynx-native` @ `91c81f4e53167cafdebdd0e3ba23829ccb8101c0` / published APK `lynx-v2-debug-91c81f4` (see Next #161 **MERGED** as PR #203). Current tip `ac78413d08fef1d09b5baf4c4e054d952a242d96` / published APK `lynx-v2-debug-ac78413` (Next #165 **MERGED** as PR #208; see Next #166).
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. No 真机过 claim. 真机残差 unchanged at write on `lynx-v2-debug-841d2e7`; current confirm APK is `lynx-v2-debug-ac78413` (Next #166).
- Deferred: host file picker / camera / clipboard File ingest; Cap IntersectionObserver; `@dnd-kit` / MobileWindowMotion / iPad / Cap toast; DOM `useAssistantContactAutoFollow` (LegendList `maintainScrollAtEnd` stays); merge to main.

## Notes — Docs tip APK/SHA honesty post-#160 (Next #161 / 2026-09-22) — **MERGED** as PR #203

- Cap skim pending / **NO_NEW** expected from prior post-#160 residual list (host file picker/camera/clipboard File ingest; Cap IntersectionObserver read-mark geometry; deferred `@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk; DOM `useAssistantContactAutoFollow` leftover — LegendList `maintainScrollAtEnd` stays) + this docs-only hop. Next #158 Cap Assistant contact transcript and Next #160 contact-* SSE stay 代码接上 (`LynxContactConversationScreen`), not 三关 landed / not 真机过. Settings search LynxInput stays 代码接上 (Next #139). Incremental leftovers unchanged: Scheduled history first page 代码接上 (load-more/`nextCursor` polish); session-row badges / Files image/markdown polish/host.
- Tip / prerelease APK: `lynx-v2-debug-91c81f4` (work/lynx-native @ `91c81f4e53167cafdebdd0e3ba23829ccb8101c0`; PR #202 / Next #160 MERGED; release exists; asset `openchamber-lynx-debug-91c81f4.apk`; published ~2026-09-22T14:32:28Z). Was stale `lynx-v2-debug-841d2e7` / tip `841d2e71644a6513106f5b10091f280c065bac11`. Tip delta `841d2e7`→`91c81f4` is docs Next #159 merge (`c537446275323648c5875933e991ebf718cdbfaa`) then product Next #160 merge (PR #202), then Mobile CI published this APK. Lynx Mobile CI SUCCESS on tip `91c81f4` (run 35740307714). lynx-ci green on tip `91c81f4` (run 35740307716). Do not invent a newer APK SHA than the published tag. applicationId stays `com.yee94.openchamber.lynx.debug`.
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged at write: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-91c81f4`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).
- **MERGED** as PR #203 — tip advanced to `618ae12a5450a633f716cc95fb4e24dbe9b9844b` / published APK `lynx-v2-debug-618ae12` (see Next #162 **MERGED** as PR #204). Current tip `ac78413d08fef1d09b5baf4c4e054d952a242d96` / published APK `lynx-v2-debug-ac78413` (Next #165 **MERGED** as PR #208; see Next #166).

## Notes — Docs tip APK/SHA honesty post-#161 (Next #162 / 2026-09-22) — **MERGED** as PR #204

- Cap skim pending / **NO_NEW** expected from prior post-#160 residual list (host file picker/camera/clipboard File ingest; Cap IntersectionObserver read-mark geometry; deferred `@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk; DOM `useAssistantContactAutoFollow` leftover — LegendList `maintainScrollAtEnd` stays) + this docs-only hop. Next #158 Cap Assistant contact transcript and Next #160 contact-* SSE stay 代码接上 (`LynxContactConversationScreen`), not 三关 landed / not 真机过. Settings search LynxInput stays 代码接上 (Next #139). Incremental leftovers unchanged: Scheduled history first page 代码接上 (load-more/`nextCursor` polish); session-row badges / Files image/markdown polish/host.
- Tip / prerelease APK: `lynx-v2-debug-618ae12` (work/lynx-native @ `618ae12a5450a633f716cc95fb4e24dbe9b9844b`; PR #203 / Next #161 MERGED; release exists; asset `openchamber-lynx-debug-618ae12.apk`; published ~2026-09-22T17:07:05Z). Was stale `lynx-v2-debug-91c81f4` / tip `91c81f4e53167cafdebdd0e3ba23829ccb8101c0`. Tip delta `91c81f4`→`618ae12` is docs-only (PR #203 merge only), then Mobile CI published this APK. Lynx Mobile CI SUCCESS on tip `618ae12` (run 35757908050). lynx-ci green on tip `618ae12` (run 35757908152). Do not invent a newer APK SHA than the published tag. applicationId stays `com.yee94.openchamber.lynx.debug`.
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged at write: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-618ae12`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).
- **MERGED** as PR #204 — tip advanced to `611d5433b2b6529243115b7dfdf111a77110c8f9` / published APK `lynx-v2-debug-611d543` (asset `openchamber-lynx-debug-611d543.apk`; published ~2026-09-22T22:45:51Z; see Next #163).

## Notes — Docs tip APK/SHA honesty post-#162 (Next #163 / 2026-09-22) — **MERGED** as PR #205

- Cap skim: **NO_NEW** Linux-closable Cap product gap (docs-only tip hop after #162). Residuals stay host-only / deferred polish (`@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk; host file picker/camera/clipboard; IntersectionObserver; Keychain/IME/Pierre/WKWebView/Mode B/Share ext/iOS IPA/FCM/Live Activity). DOM `useAssistantContactAutoFollow` leftover — LegendList 1.19 `maintainScrollAtEnd` stays; TanStack Virtual 1.18 stays out. Next #158 Cap Assistant contact transcript and Next #160 contact-* SSE stay 代码接上 (`LynxContactConversationScreen`), not 三关 landed / not 真机过. Settings search LynxInput stays 代码接上 (Next #139). Incremental leftovers unchanged: Scheduled history first page 代码接上 (load-more/`nextCursor` polish); session-row badges / Files image/markdown polish/host. Next #162 **MERGED** as PR #204.
- Tip `work/lynx-native` @ `611d5433b2b6529243115b7dfdf111a77110c8f9` (PR #204 / Next #162 MERGED). Published APK `lynx-v2-debug-611d543` (prerelease; exists; asset `openchamber-lynx-debug-611d543.apk`; published ~2026-09-22T22:45:51Z). Was `lynx-v2-debug-618ae12` (asset `openchamber-lynx-debug-618ae12.apk`; published ~2026-09-22T17:07:05Z by Mobile CI run 35757908050). Tip delta `618ae12a5450`→`611d543` is docs-only (Next #162 honesty commit + PR #204 merge). No product Cap gap in that hop. Lynx Mobile CI run `35775756905` on tip `611d543`: attempt 1 FAILURE (Android assembleRelease); attempt 2 SUCCESS and published this APK. lynx-ci green on tip `611d543` (run 35775757127). applicationId stays `com.yee94.openchamber.lynx.debug`.
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged at write: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-611d543`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).
- **MERGED** as PR #205 — tip advanced to `3d0e7c314192401061db279c431c4ba66275ce1a` / published APK `lynx-v2-debug-3d0e7c3` (asset `openchamber-lynx-debug-3d0e7c3.apk`; published ~2026-09-22T22:56:18Z; see Next #164).

## Notes — Docs tip APK/SHA honesty post-#163 (Next #164 / 2026-09-23) — **MERGED** as PR #206

- Cap skim: **NO_NEW** Linux-closable Cap product gap (docs-only tip hop after #163). Residuals stay host-only / deferred polish (`@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk; host file picker/camera/clipboard; IntersectionObserver; Keychain/IME/Pierre/WKWebView/Mode B/Share ext/iOS IPA/FCM/Live Activity). DOM `useAssistantContactAutoFollow` leftover — LegendList 1.19 `maintainScrollAtEnd` stays; TanStack Virtual 1.18 stays out. Next #158 Cap Assistant contact transcript and Next #160 contact-* SSE stay 代码接上 (`LynxContactConversationScreen`), not 三关 landed / not 真机过. Settings search LynxInput stays 代码接上 (Next #139). Incremental leftovers unchanged: Scheduled history first page 代码接上 (load-more/`nextCursor` polish); session-row badges / Files image/markdown polish/host. Next #163 **MERGED** as PR #205.
- Tip `work/lynx-native` @ `3d0e7c314192401061db279c431c4ba66275ce1a` (PR #205 / Next #163 MERGED). Published APK `lynx-v2-debug-3d0e7c3` (prerelease; exists; asset `openchamber-lynx-debug-3d0e7c3.apk`; published ~2026-09-22T22:56:18Z). Was `lynx-v2-debug-611d543` (asset `openchamber-lynx-debug-611d543.apk`; published ~2026-09-22T22:45:51Z by Mobile CI run 35775756905). Tip delta `611d5433b2b6`→`3d0e7c3` is docs-only (Next #163 honesty commit + PR #205 merge). No product Cap gap in that hop. Lynx Mobile CI SUCCESS on tip `3d0e7c3` (run 35794382976). lynx-ci green on tip `3d0e7c3` (run 35794383057). applicationId stays `com.yee94.openchamber.lynx.debug`.
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged at write: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-3d0e7c3`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).
- **MERGED** as PR #206 — tip advanced to `c73a66e57e6040d7c7e86001edc0e421f03ff1de` / published APK `lynx-v2-debug-c73a66e` (asset `openchamber-lynx-debug-c73a66e.apk`; published ~2026-09-23T04:36:07Z; see Next #165).

## Notes — Docs tip APK/SHA honesty post-#164 (Next #165 / 2026-09-23) — **MERGED** as PR #208

- Cap skim: **NO_NEW** Linux-closable Cap product gap (docs-only tip hop after #164). Residuals stay host-only / deferred polish (`@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk; host file picker/camera/clipboard; IntersectionObserver; Keychain/IME/Pierre/WKWebView/Mode B/Share ext/iOS IPA/FCM/Live Activity). DOM `useAssistantContactAutoFollow` leftover — LegendList 1.19 `maintainScrollAtEnd` stays; TanStack Virtual 1.18 stays out. Next #158 Cap Assistant contact transcript and Next #160 contact-* SSE stay 代码接上 (`LynxContactConversationScreen`), not 三关 landed / not 真机过. Settings search LynxInput stays 代码接上 (Next #139). Incremental leftovers unchanged: Scheduled history first page 代码接上 (load-more/`nextCursor` polish); session-row badges / Files image/markdown polish/host. Next #164 **MERGED** as PR #206.
- Tip `work/lynx-native` @ `c73a66e57e6040d7c7e86001edc0e421f03ff1de` (PR #206 / Next #164 MERGED). Published APK `lynx-v2-debug-c73a66e` (prerelease; exists; asset `openchamber-lynx-debug-c73a66e.apk`; published ~2026-09-23T04:36:07Z). Was `lynx-v2-debug-3d0e7c3` (asset `openchamber-lynx-debug-3d0e7c3.apk`; published ~2026-09-22T22:56:18Z by Mobile CI run 35794382976). Tip delta `3d0e7c314192`→`c73a66e` is docs-only (Next #164 honesty commit + PR #206 merge). No product Cap gap in that hop. Lynx Mobile CI SUCCESS on tip `c73a66e` (run 35818527655). lynx-ci green on tip `c73a66e` (run 35818527681). applicationId stays `com.yee94.openchamber.lynx.debug`.
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged at write: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-c73a66e`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).
- **MERGED** as PR #208 — tip advanced to `ac78413d08fef1d09b5baf4c4e054d952a242d96` / published APK `lynx-v2-debug-ac78413` (asset `openchamber-lynx-debug-ac78413.apk`; published ~2026-09-23T10:34:53Z; see Next #166).

## Notes — Docs tip APK/SHA honesty post-#165 (Next #166 / 2026-09-23)

- Cap skim: **NO_NEW** Linux-closable Cap product gap (docs-only tip hop after #165). Residuals stay host-only / deferred polish (`@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk; host file picker/camera/clipboard; IntersectionObserver; Keychain/IME/Pierre/WKWebView/Mode B/Share ext/iOS IPA/FCM/Live Activity). DOM `useAssistantContactAutoFollow` leftover — LegendList 1.19 `maintainScrollAtEnd` stays; TanStack Virtual 1.18 stays out. Incremental leftovers unchanged: Scheduled history first page already 代码接上 (Cap load-more/`nextCursor` + per-task filter stay polish); Settings search field 代码接上 (`LynxInput` + `bindinput`, Next #139), not 三关 landed; session-row question/permission badges + Files image/markdown preview stay polish / host. SessionsSheet/ProjectsHome row Stop is extra affordance (`abortSession` already wired from composer/goal). Active Show more beyond session-index snapshot is the same class as Scheduled history `nextCursor` (index-driven list). `GET /api/client-auth/connection/candidates` is post-connect lifecycle leftover, not welcome-blocking. Next #158 Cap Assistant contact transcript and Next #160 contact-* SSE stay 代码接上 (`LynxContactConversationScreen`), not 三关 landed. Next #165 **MERGED** (PR #208).
- Tip `work/lynx-native` @ `ac78413d08fef1d09b5baf4c4e054d952a242d96` (PR #208 / Next #165 MERGED). Published APK `lynx-v2-debug-ac78413` (prerelease; exists; asset `openchamber-lynx-debug-ac78413.apk`; published ~2026-09-23T10:34:53Z). Was `lynx-v2-debug-c73a66e` (asset `openchamber-lynx-debug-c73a66e.apk`; published ~2026-09-23T04:36:07Z by Mobile CI run 35818527655). Tip delta `c73a66e57e60`→`ac78413` is docs-only (Next #165 honesty commit + PR #208 merge). No product Cap gap in that hop. Lynx Mobile CI SUCCESS on tip `ac78413` (run 35848995165; lynx type-check + vitest + rspeedy green as a job under that Mobile CI run). Separate `lynx-ci.yml` did not produce a new run on `ac78413` (last separate lynx-ci success remains on `c73a66e`: run 35818527681). applicationId stays `com.yee94.openchamber.lynx.debug`.
- Product **NOT DONE** / 三关未齐 / not EXHAUSTED. 真机残差 unchanged: Connect welcome **card chrome** confirm still open for Yee on published APK `lynx-v2-debug-ac78413`. No 真机过 claim. Host-only unchanged (Keychain/camera/IME/Pierre/WKWebView/Mode B/Share ext/ShareReceiverActivity/iOS IPA/FCM for lynx.debug/Live Activity/clipboard host).
