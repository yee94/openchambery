...
- CI: `.github/workflows/lynx-ci.yml` + `lynx-mobile-ci.yml` are **live** on tip (`work/lynx-native`). lynx-ci = Linux type-check + vitest + rspeedy (green). lynx-mobile-ci = Android sideload APK (`assembleRelease` + `.debug` applicationId) + prerelease `lynx-v2-debug-*` (e.g. `lynx-v2-debug-09b3853`). Android first-paint cream/globalProps mitigation is 代码接上 — **not** 真机过. iOS IPA / simulator still missing. **Do not claim 真机过** from CI alone. Product NOT DONE / 三关未齐.
