...
- Tip / APK honesty: write-time base tip `work/lynx-native` @ `ceb8c30a9da2f97fc9360b189f2fd6ef007397b0` (PR #199 / Next #157 MERGED). Published APK at write time: `lynx-v2-debug-4d4ff10` (do **not** invent `lynx-v2-debug-ceb8c30`). **MERGED** as PR #200 — current tip `841d2e71644a6513106f5b10091f280c065bac11` / published APK `lynx-v2-debug-841d2e7` (see Next #159). Product **NOT DONE** / 三关未齐 / not EXHAUSTED. No 真机过 claim.
- Residual: host attachment picker/camera; Cap IntersectionObserver read-mark geometry; no Cap `contact/cards` / `contact/dm` / `contact-*` SSE on this tip; Cap toast / `@dnd-kit` / iPad / host-only unchanged.

## Notes — Docs tip APK/SHA honesty post-#158 (Next #159 / 2026-09-22)

- Cap skim pending / **NO_NEW** expected from prior post-#158 residual list + this docs-only hop. Next #158 Cap Assistant contact transcript stays 代码接上 (`LynxContactConversationScreen`), not 三关 landed / not 真机过. Settings search LynxInput stays 代码接上 (Next #139). Incremental leftovers unchanged: Scheduled history first page 代码接上; session-row badges / Files image/markdown polish/host; deferred `@dnd-kit` / MobileWindowMotion / iPad / Cap toast / bulk.
