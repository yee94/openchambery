...
| Row | Status | Notes |
|---|---|---|
| Track (CODE) | **code landed** | LegendList transcript (`initialScrollAtEnd` / `maintainScrollAtEnd` / MVCP); load `GET .../messages?turns=6`; Send `prompt_async` / Stop `abort`; draft materialize `POST /api/session` then prompt; events prefer `/api/global/event/ws` → SSE → poll-only reconnect fallback; streaming markdown paced 64ms (Android 128ms) with incomplete-fence isolation; composer text+send/stop only (no mic/TTS). **Residuals landed this slice:** minimal expandable tool cards (Cap ToolPart/ProgressiveGroup short-description subset; no Used fold / LatticeOrb / nested task chrome); reasoning disclosure (default collapsed, stream auto-expands); queue chips via `GET /api/openchamber/message-queue` + scope page, admit-when-busy (`POST .../items`) when directory known, remove chip. Unit + 关3 perf harness green locally. Still missing: slash/@/# autocomplete, attachments + HEIC/pickers, context usage ring, process Used fold / skill groups / question cards, queue edit/reorder/DnD. Native composer glass stays Track 8 (do not clobber Expo UI pixel commits). |
| CI green | **CI green** @ `8d0043d1` | Expo Mobile CI lint+typecheck+vitest covers `apps/mobile_expo` — https://github.com/yee94/openchambery/actions/runs/34019907976. Tip docs SHA `4e6fd48e` (docs-only after product proof `8d0043d1`). |
| 真机过 | missing | Do not mark from Linux VM |
