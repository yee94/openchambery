...
| CI green | **CI green** @ `bc24bd74` | Expo Mobile CI full workflow (lint+typecheck+vitest + Android assembleRelease sideload + emulator smoke + prerelease) green — https://github.com/yee94/openchambery/actions/runs/34698745345. Prior `a6ca019b` / 34046861914; `67cb7f2e` / 34034421349 (logcat past splash); `63e43620` / 34027594765 (embed JS); `1436c630` / 34024670880 (no bundle) superseded. |
| 真机过 | missing | |
...
| Row | Status | Notes |
|---|---|---|
| Lint / typecheck / unit tests workflow | **CI green** @ `bc24bd74` | Tip run https://github.com/yee94/openchambery/actions/runs/34698745345 — Lint/typecheck/unit tests **green**. Prior `a6ca019b` / 34046861914; `67cb7f2e` / 34034421349; `63e43620` / 34027594765; `1436c630` / 34024670880 superseded. |
| Device binary jobs | **CI green** (Android emulator logcat); iOS residual | assembleRelease sideload + API 30 smoke @ `bc24bd74` (run 34698745345). Logcat past splash YES. Prior `a6ca019b` / 34046861914; `67cb7f2e` / 34034421349. iOS simulator residual: macos runner + CocoaPods + Track 8 native modules not wired. |
| Debug APK + prerelease link | **CI green** (logcat) | Prerelease `expo-v2-debug-bc24bd7` — https://github.com/yee94/openchambery/releases/tag/expo-v2-debug-bc24bd7. assembleRelease+.debug; emulator logcat past splash YES (`Running "main"` + Displayed). Prior `a6ca019b` / `expo-v2-debug-a6ca019`; `67cb7f2e`; `63e4362` assembleDebug hung / crashed; `1436c63` no bundle. |
| 真机过 | missing | |
...
| Prerelease debug APK (`applicationIdSuffix .debug`, label **OpenChamber Expo**, embedded JS) | **CI green** @ `bc24bd74` | https://github.com/yee94/openchambery/releases/tag/expo-v2-debug-bc24bd7 — assembleRelease sideload + embedded Hermes; side-by-side id/label unchanged (not `/releases/latest`). Prior `a6ca019b` / `67cb7f2e` / `63e43620` / `1436c630` superseded. |
| Signed release workflow | not started | Existing secret names only; debug keystore for Track 9 |
| CI lint / typecheck on `work/expo-native` | **CI green** @ `bc24bd74` | https://github.com/yee94/openchambery/actions/runs/34698745345 (lint+typecheck+vitest) |
| Android debug CI binary | **CI green** @ `bc24bd74` | Run 34698745345 Android job + prerelease green (assembleRelease + emulator logcat). Prior `a6ca019b` / `67cb7f2e` / `63e43620` (embed) / `1436c630` (Metro-dependent) superseded. |
...
**CODE EXHAUSTED** for feature tracks 1–8 @ tip `bc24bd74` (`bc24bd74448780fefacf6234596ae4e5e34de2a1`). Track 9 Android CI **green** (https://github.com/yee94/openchambery/actions/runs/34698745345; prerelease https://github.com/yee94/openchambery/releases/tag/expo-v2-debug-bc24bd7). **Sep 12** Cap-parity Markdown landed in chat bubbles + reasoning/tool card bodies; UI optical gaps tightened. Residuals remain **真机过** + device + polish + iOS sim/signed as already documented. **真机过 still missing** on all nine tracks. Do not invent new product tracks from polish pixels.
