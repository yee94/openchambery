...
Track tip (docs honesty, Next #153): `work/lynx-native` @ `c3a062a134a9796041ea605eb2e1acc66a2b289d` (PR #194 / Next #152 MERGED). Published APK: `lynx-v2-debug-c3a062a` (prerelease; exists; asset `openchamber-lynx-debug-c3a062a.apk`; was stale `lynx-v2-debug-309af29` / tip `309af29373d68d7d4ea3757af9adf4896946490f`). Tip delta `309af29`→`c3a062a` is docs-only. Lynx Mobile CI SUCCESS on tip `c3a062a` (run 35646713115). Do not invent a newer APK than published. Product **NOT DONE** / 三关未齐 / not EXHAUSTED. Next #90 archived-sessions and Next #91 Assistant unread are landed. Next #139 Settings search LynxInput is 代码接上, not 三关 landed. 真机残差: Connect welcome **card chrome** confirm for Yee on published APK `lynx-v2-debug-c3a062a`. Host-only unchanged.
