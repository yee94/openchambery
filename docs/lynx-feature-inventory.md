...
Track tip (docs honesty, Next #166; product Next #160 **MERGED** as PR #202; Next #161 **MERGED** as PR #203; Next #162 **MERGED** as PR #204; Next #163 **MERGED** as PR #205; Next #164 **MERGED** as PR #206; Next #165 **MERGED** as PR #208): tip `work/lynx-native` @ `ac78413d08fef1d09b5baf4c4e054d952a242d96` (PR #208 / Next #165 MERGED). Published APK `lynx-v2-debug-ac78413` (prerelease; exists; asset `openchamber-lynx-debug-ac78413.apk`; published ~2026-09-23T10:34:53Z). Was `lynx-v2-debug-c73a66e` (asset `openchamber-lynx-debug-c73a66e.apk`; published ~2026-09-23T04:36:07Z by Mobile CI run 35818527655). Tip delta `c73a66e57e60`→`ac78413` is docs-only (Next #165 honesty commit + PR #208 merge). No product Cap gap in that hop. Lynx Mobile CI SUCCESS on tip `ac78413` (run 35848995165; lynx type-check + vitest + rspeedy green as a job under that Mobile CI run). Separate `lynx-ci.yml` did not produce a new run on `ac78413` (last separate lynx-ci success remains on `c73a66e`: run 35818527681). Cap skim: **NO_NEW** Linux-closable Cap product gap. Next #158 Cap Assistant contact transcript and Next #160 contact-* SSE previews (`openchamber:contact-turn-start` / `contact-bubble-delta` / `contact-turn-end` + optimistic bubbles on the contact transcript) are 代码接上 (`LynxContactConversationScreen`), not 三关 landed. The prior "no contact-* SSE on tip" residual is struck. LegendList 1.19 `maintainScrollAtEnd` stays; TanStack Virtual 1.18 stays out. Product **NOT DONE** / 三关未齐 / not EXHAUSTED. Next #90 archived-sessions and Next #91 Assistant unread are landed. Next #139 Settings search LynxInput is 代码接上, not 三关 landed. 真机残差: Connect welcome **card chrome** confirm for Yee on published APK `lynx-v2-debug-ac78413`. Residuals stay host-only / deferred polish. applicationId stays `com.yee94.openchamber.lynx.debug`.
