...
Track tip (docs honesty, Next #161; product Next #160 **MERGED** as PR #202): tip `work/lynx-native` @ `91c81f4e53167cafdebdd0e3ba23829ccb8101c0` (PR #202 / Next #160 MERGED). Published APK: `lynx-v2-debug-91c81f4` (prerelease; exists; asset `openchamber-lynx-debug-91c81f4.apk`; published ~2026-09-22T14:32:28Z). Was stale published cite `lynx-v2-debug-841d2e7` / tip `841d2e71644a6513106f5b10091f280c065bac11` — do **not** invent a newer APK than `lynx-v2-debug-91c81f4`. Tip delta `841d2e7`→`91c81f4` is docs Next #159 merge (`c537446275323648c5875933e991ebf718cdbfaa`) then product Next #160 merge (PR #202), then Mobile CI republished the tip APK. Lynx Mobile CI SUCCESS on tip `91c81f4` (run 35740307714). lynx-ci green on tip `91c81f4` (run 35740307716). Next #158 Cap Assistant contact transcript and Next #160 contact-* SSE previews (`openchamber:contact-turn-start` / `contact-bubble-delta` / `contact-turn-end` + optimistic bubbles on the contact transcript) are 代码接上 (`LynxContactConversationScreen`), not 三关 landed. The prior "no contact-* SSE on tip" residual is struck. Product **NOT DONE** / 三关未齐 / not EXHAUSTED. Next #90 archived-sessions and Next #91 Assistant unread are landed. Next #139 Settings search LynxInput is 代码接上, not 三关 landed. 真机残差: Connect welcome **card chrome** confirm for Yee on published APK `lynx-v2-debug-91c81f4`. Host-only unchanged. applicationId stays `com.yee94.openchamber.lynx.debug`.
