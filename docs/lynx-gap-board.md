...
| Assistant catalog + conversation chrome | `MobileAssistantTab.tsx`, assistants snapshot | `assistants/*` → `GET /api/openchamber/assistants/snapshot` (+ ensure-session hook). Catalog tab; open conversation reuses `LynxChatScreen`. Unread badge + mark-all + `POST …/contact/read` in Next #91. No invented ASR. Missing session → labeled stub (not fake chat id). |
...
| ~~Settings search + **all 22 mobile slugs**~~ | `MOBILE_SETTINGS_PAGE_SLUGS` (Cap main) | **代码接上** home search + grouped rows + push pages (`archived-sessions` page in Next #90) |
...
6. ~~**Settings home + slug map**~~ — 代码接上 search + 22 Cap main rows (`archived-sessions` page body in Next #90). Bodies: wired/list/stub in settings-ci slice. Remaining: rich entity editors.
