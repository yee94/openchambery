...
export function parseShareDraft(raw: unknown): ShareDraft | null {
  if (!raw || typeof raw !== 'object') return null;
  const o = raw as Record<string, unknown>;
  if (o.version !== 1) return null;
  if (typeof o.draftID !== 'string' || !o.draftID.trim()) return null;
  if (o.source !== 'android-share') return null;
  if (!Number.isFinite(o.createdAt) || !Number.isFinite(o.expiresAt)) return null;
  const attachments = Array.isArray(o.attachments)
    ? o.attachments
        .map((item) => {
  ...
  return {
    ...
    serverInstanceID: typeof o.serverInstanceID === 'string' ? o.serverInstanceID : undefined,
    assistantID: typeof o.assistantID === 'string' ? o.assistantID : undefined,
    name: typeof o.name === 'string' ? o.name : undefined,
    avatarSeed: typeof o.avatarSeed === 'string' ? o.avatarSeed : undefined,
    serverLabel: typeof o.serverLabel === 'string' ? o.serverLabel : undefined,
    connectionKey: typeof o.connectionKey === 'string' ? o.connectionKey : undefined,
    text: typeof o.text === 'string' ? o.text : undefined,
    attachments,
    source: 'android-share',
    createdAt: Number(o.createdAt),
...
export async function listShareDrafts(): Promise<ShareDraft[]> {
  const { shareNative } = await import('openchamber-system-shell');
  if (!shareNative.listDrafts) return [];
  const raw = await shareNative.listDrafts();
  return raw.map(parseShareDraft).filter((x): x is ShareDraft => !!x);
}

export async function cancelShareDraft(draftID: string): Promise<void> {
  const { shareNative } = await import('openchamber-system-shell');
  if (!shareNative.cancelDraft) return;
  await shareNative.cancelDraft(draftID);
}
