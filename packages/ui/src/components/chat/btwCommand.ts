 ...
 * Bare `/btw` only opens the side conversation; nothing is requested until a
 * question is actually sent (inline here or from the panel composer).
 */
export function submitBtwCommand(options: {
  ...
  open: () => void;
  clearText: () => void;
  notify: (reason: 'sessionRequired' | 'plainTextRequired') => void;
}): boolean {
  ...
  if (question) void options.ask(options.scope, question);
  options.clearText();
  options.open();
  return true;
}
