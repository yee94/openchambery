...
- `processToolGrouping.ts`
  - Unified process fold helpers: `collectConsecutiveProcessTools`, `summarizeProcessTools`, `summarizeUsedToolDiffs`, `isProcessGroupActive`.
  - Body text / skill / task / question settle the live label. Explore and used tools do not.
...
- `usedToolGrouping.ts`
  - Used-class count helpers consumed by the unified process fold: `summarizeUsedTools`, `summarizeUsedToolDiffs`.
  - Count keys: edit (`edit` / `write` / `apply_patch` / …), command (`bash` / `shell` / …), other (custom / MCP / web / todo).
...
- `toolRowChrome.ts`
  ...
  - `getToolGroupExpandedListClass` spaces Used / Skill fold children with the same mobile `py-1` / desktop `py-1.5` step as `getToolRowBlockClass` (`gap-1 pt-1` vs `gap-1.5 pt-1.5`).
  - `TOOL_ROW_CHIP_GEOMETRY_CLASS` (`rounded-lg px-2 py-0.5`) is the tool-row chip geometry. Assistant error / retry / abort rows no longer use it: they are borderless full-width meta rows owned by `MessageBody` (see chat `DOCUMENTATION.md`).
...
- `toolRenderUtils.ts`
  - Core classification helpers:
    ...
    - `isUsedGroupTool` (edit / command / other; not explore / skill / task / question)
    - `isProcessGroupTool` (explore + used; the unified process fold)
    - `isToolPartActive` / `isToolPartSettled` lifecycle evidence shared by flat rendering, static rows, process groups, skill groups, and expandable rows
  - If a tool should switch between static vs expandable, or join the process / Skill grouping, change it here.
...
- Each sorted turn uses one Activity container with the same full-width clickable header DOM, layout, typography, spacing, mobile behavior, and ARIA disclosure contract while collapsed and expanded. Live-active presentation (latest turn while the session is working) always starts expanded for untouched turns regardless of `activityRenderMode`; settled turns use that setting for the untouched default (`summary` open, `collapsed` closed). The newest turn stays open until the projection confirms its final body (`turn.hasConfirmedFinalBody`: terminal stop with zero continuation tools, no error, plus model-produced text); confirmation of an untouched live turn then re-applies the setting so `collapsed` auto-folds. Every click reverses the current per-turn value, and touched values survive later setting and disposition changes. Before toggling, the header captures its viewport top; the next layout synchronously compensates `[data-scrollbar="chat"]` and performs one frame-level correction after virtualizer measurement, keeping the control under the pointer. Collapsed turns render only this header; expanded turns render the complete ordered detail timeline. Live non-compaction turns do **not** paint the Working disclosure header (`paintDisclosureHeader` is false while `completionDisposition === 'active'` and the turn is not compaction): only the force-expanded tool/reasoning rows render, with no indent rail. Compaction still shows localized Compacting text with the info-token shimmer while active. Normal and abnormal completed headers keep the foldable Processed / Compaction complete chrome plus the authoritative duration across both disclosure states. Live elapsed while working remains owned only by `WorkingPlaceholder` on the status row. An undefined completion disposition shows localized Processing details. Task inputs contribute their robustly parsed `subagent_type` to unbordered square avatars (up to two on mobile, three on desktop) separated by a small gap. Localized status text uses the authoritative total: active headers count running and pending Tasks, and completed headers count every participating Task. The disclosure header is a single-line full-width flex row: the left cluster (icon + status + duration) uses `flex-1` to absorb free space; the right trailer (agents when present + chevron) uses `ml-auto` so it stays on the trailing edge. Desktop keeps chip `px-2` so hover wash padding is symmetric; mobile only drops right padding (`pr-0` + slight chevron `-mr`) so the glyph has no dead trailing slot.
...
- `edit` / `multiedit` / `write` stay in `ToolPart` for title + path + diff-stats chrome and use **non-expandable file navigation**. Web/Desktop and dedicated mobile `edit` / `multiedit` / `write` clicks open the selected tool's single-file patch, including paths outside the workspace such as `/tmp` (`write` / `create` / `file_write` synthesize a full added-file patch from the written content; a single outside-workspace or write patch path is normalized to the click target so the Changes view can find the file). The patch is already on the tool part, so this does not read the outside file or request an outside-file grant. A click with no renderable patch still opens the outside file itself; `apply_patch` clicks open every renderable file patch from that tool invocation. The initial target scrolls to its first changed line, and the existing bounded stacked-diff policy limits how many large patches mount at once. Web/Desktop renderable-patch fallback opens the target file from the owning turn, or the working-tree change for writes (brand-new files are often absent from the turn snapshot). Dedicated mobile uses the closable Motion sheet on phones and the right Changes panel on iPad; standard file Changes handles edit fallback, while apply-patch fallback presents every file from the owning turn. VS Code opens the primary file in its native diff editor. Patch records lacking a complete renderable file set open the owning turn's complete diff. OpenCode 2 stores that path on `files[].file` (workspace-relative) while edit `input.path` is often absolute; `toolFilePathsMatch` treats those as the same file so the click still opens that patch instead of an empty last-turn Changes view. `patchFilePath` also accepts `movePath`, `relativePath`, and `filePath`. A one-file patch row shows `+added/-removed` from that file's `additions` and `deletions`, or from its patch text when those counts are absent. V2 does not ship `multiedit`; a historical part with that name uses the edit label. The shell tool arrives as `shell` and renders through the `bash` command row. No chevron / expanded diff body.
...
- Markdown file references classify file content before opening. Morphdom wraps path tokens during `decorateMarkdown` so it owns the underline structure; annotation only stats and sets `data-openchamber-file-link`. Markstream wraps the same tokens during the React render of `text` / `inline_code` / `link` nodes rather than mutating the tree after paint. Binary files use the desktop system handler. Image paths open the shared `image-preview` popup. HTML/HTM paths open the Files/mobile webview preview (`viewerMode: 'preview'`), not the source editor or VS Code `editor.openFile`.
- Mobile surfaces annotate previewable text, image, and HTML paths. Other binary paths remain plain text. Direct/LAN HTML preview uses a sandboxed iframe on `/api/fs/serve`. Relay cannot load iframe `src` through the tunnel, so the document is fetched with `runtimeFetch` (or the already-read file body on desktop) and inlined as `srcDoc`.
