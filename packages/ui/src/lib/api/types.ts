...
export interface GitAPI {
  ...
  generateCommitMessage(directory: string, files: string[], options?: { zenModel?: string; providerId?: string; modelId?: string; signal?: AbortSignal }): Promise<{ message: GeneratedCommitMessage }>;
  generatePullRequestDescription(
    directory: string,
    payload: { base: string; head: string; context?: string; zenModel?: string; providerId?: string; modelId?: string; signal?: AbortSignal }
...
export interface FilesAPI {
  ...
  statFile?(path: string, options?: FileReadOptions): Promise<{ path: string; isFile: boolean; size: number; mtimeMs?: number; isBinary?: boolean }>;
...
export interface SettingsPayload {
  ...
  questionAutoDelegateEnabled?: boolean;
