...
`context-token-baseline.ts` owns the backward baseline scan for web/Electron headers, mini chat, VS Code and mobile. Running and failed compactions are skipped, including their request token counts; only a completed checkpoint invalidates the earlier window. A checkpoint with unavailable status is conservatively unknown. Compaction request tokens are never treated as the resulting context size. The next ordinary assistant with nonzero token usage replaces the pending baseline.

`buildSessionContextUsage` represents invalidated usage with `pending: true` (numeric fields are placeholders, not measured zero). All UI consumers use `useSessionContextUsage` / `useTranscriptContextUsage`, a repository selector that observes both messages and parts while publishing only changed usage fields. A part-only compaction completion updates the control; summary/text deltas do not rerender it. `ContextUsageDisplay` keeps the control visible with localized “Awaiting update”, neutral styling and no numeric ARIA value. Mobile maps pending usage to a `null` total (`0` means no baseline) and shows the pending label in its context panel. No estimation, polling or persisted fallback is introduced. Refresh/revisit derives the same state from authoritative projected compaction parts. Regression coverage includes `context-usage-display.test.tsx`, which mounts the controls against the Query-backed repository and real live-event reducer.

## Message content order

Within a message, native `content[]` / projected `parts[]` order is authoritative. Part IDs are identity only: tool `call-*` IDs and generated `msg_*:text:N` / `reasoning:N` IDs cannot rank content. Production page loading, materialization, optimistic insertion, and both repository adapters preserve array order. Live updates and removals locate parts by identity, replace in place, and append newly observed parts; optimistic replacement keeps its slot. Partial materialization retains omitted live parts before their next surviving neighbor (or at the tail when none remains), without reordering the incoming snapshot. These shared rules apply to web, Electron, VS Code, hosted mobile, and Capacitor.

An authoritative refresh repairs previously ID-sorted in-memory or durable-seeded content; settled writes persist the corrected array, whose content fingerprint includes order. Existing cache records cannot reconstruct source order on their own and may retain the old order until authority refresh succeeds. No identity or storage schema changes are needed. Regression coverage: `transcript-part-order.test.ts`.

## OpenCode 2 shell, form replies, and stream timing

### Interrupt acknowledgement versus execution settlement

`session-interrupt.ts` owns the bounded stop race: HTTP acknowledgement, a newly observed idle state, or a five-second unconfirmed outcome. It unsubscribes and aborts transport best-effort on settlement, even when a bridge ignores cancellation. `abortCurrentOperation` deduplicates simultaneous callers by captured directory store, session and runtime/execution generation. Failed/unconfirmed operations roll back only their matching queue-abort block and presentation, preserve execution authority, and show localized retry feedback. A replaced runtime/store or newer execution cannot receive a stale acknowledgement or rollback.

An accepted request writes only `session_interrupt_acknowledged_at`; `session_status` and its authoritative observation time remain unchanged. `useSessionStatus` / `useSessionStatusObservedAt` project that receipt for interaction/activity display. Queue dispatch and history mutation code continue reading raw store authority and the existing server-idle queue gate. `session_execution_version` increments for native execution starts and local direct sends (including already-materialized sends), invalidating previous stops without relying on millisecond clock uniqueness. These starts, terminal events and cache eviction remove the receipt; newer authoritative observation/snapshot clocks supersede it after reconnect. Receipt/version maps are runtime memory only and follow the child-store lifecycle, with no persisted format or transport changes.
...
`TranscriptTransportPage.coverageMessageIDs` carries projection coverage separately from context or exact-parent enrichment. Production merge/anchor extension preserves this coverage, and `refreshFromAuthority` passes it to `reconcileFetched`: only projection IDs can establish the start of the range in which missing rows imply deletion. An empty coverage array on a system-only page is not an empty authoritative transcript. Context can refresh displayed rows but cannot erase the already-loaded middle of a conversation it omitted. Complete projection snapshots still own genuine removals, and in-flight touched/optimistic rows retain their existing protection. `transcript-history-retention.test.ts` covers 65 rows across four pages, context-enriched system tails, concurrent prepend/refresh/live send in both completion orders, failures, stationary cursors, and successful retry.
...
Explicit Question dismissal uses the scoped SDK session interrupt with continuation disabled before any form cancellation. This supplies the native user interruption reason; the existing interrupted reducer settles idle and clears recovery. `form.cancel` is only an idle-orphan fallback after `interrupted: false`. Failure preserves the pending question, and runtime changes fence late local cleanup and follow-up requests. See `session-actions.test.ts` dismissal regressions.
...
The inbox overlay retains all admitted user items for reconciliation, but queue visibility uses per-item `wasQueued` provenance. Remember, authoritative replacement, and delivery updates set it on actual queue delivery and preserve it across queue→steer promotion. A queue POST whose body omits delivery still stamps `wasQueued` before any echoed steer. Fresh direct steer items never render queue chips. `session.inbox.delivered` or authority-confirmed transcript identities retire queue/steer chips immediately and record a consumed receipt, fencing late POST/PATCH responses and older inbox snapshots. 引导中 describes an unconsumed message, not the lifetime of the AI turn; busy/idle state neither retains consumed chips nor consumes pending ones. Cancel still retires immediately. The field is runtime memory only and introduces no persisted queue state. This lifecycle is shared across web, Electron, VS Code, hosted mobile and Capacitor.
...
### Native projection SWR and revisit continuity

Selection's cold path enters `ensureTranscriptInitial`, sharing the repository's durable seed, request state and single flight with mounted observers. Leaving a session does not invalidate its scoped response; runtime/projection/read-epoch retirement does. Selection-dependent todo/status side effects still stop after navigation.

Hot ensure, explicit refresh, observe checks and reconnect use the same `refreshFromAuthority` touched-message reconciliation. Concurrent refreshes share one promise per runtime/directory/session/read epoch/projection revision. Unchanged message/part references survive; a failed fetch preserves the prior transcript. Repository disposal retires late refresh completions.

Recovery marks cached scopes stale, prioritizes viewed/busy/observed sessions, and limits per-directory concurrency to two. It uses native projection through the repository, never Host anchor scans, continuation rounds or destructive recovery resets. Inactive scopes refresh on observation; successful observe checks have a 60-second throttle and failures remain retryable. A disjoint new tail retains cached older rows but adopts the new native cursor so the missing middle remains pageable. Gap pages insert chronologically among retained rows rather than ahead of the entire cache.

Behavior tests: `session-actions.test.ts` (A→B→A), `transcript-repository-query-adapter.test.ts` (SSE races, shared refresh, disposal), `session-transcript-reconnect-compensation.test.ts` (native recovery scheduling), `session-transcript-long-gap.runtime.test.js` (100-message gap and cursor traversal).
...
| `session-transcript-reconnect-compensation.ts` | Native recovery scheduler — stale marking, viewed/busy/observed demand, per-directory concurrency, native repository refresh, failed-fetch retention and retry, 60s successful observe-check throttle |
...
| `session-transcript-reconnect-compensation.test.ts` | First-ready skip, priority set, concurrency, failure/retry, inactive observation, generation cancellation and runtime registration |
...
- Native reconnect compensation is production-registered. Ready with
  `isReconnect:true` refreshes the viewed/busy/observed set after replay;
  first ready skips gap work. Disconnect capture marks cached scopes stale
  without copying messages. Recovery uses `refreshFromAuthority` for warm
  scopes and `ensureInitial` for cold scopes. Neither clears cached content.
  Non-stale observe checks reuse the same refresh with a 60-second successful
  check throttle. A failed check preserves rows and remains retryable.
...
- **Enter-and-sync hot revalidate.** A hot cache (canonical pages present,
  ...
  - `ensureInitial` on a hot cache that is **not** `activeRegistry`-retained
    joins `refreshFromAuthority`. It snapshots message/part references before
    GET and reconciles per touched id; SSE on B cannot freeze untouched A.
    Automatic and explicit refresh share the same scope flight.
  ...
  - `refreshFromAuthority` is a user-triggered reconcile (not reset); see above.
   ...
   Open / focus / 「同步消息」 force GET the v2 projection tail and merge as
   `{type:"http-page", purpose:"reconcile-page"}` (non-optimistic in-range
   absences deleted only after a non-stale merge; optimistic rows protected).
...
- `session-status-reconciliation.ts` owns authoritative status snapshot fuse and
  ...
  store can recover live status from its authoritative status endpoint. Native
  transcript recovery never resets the cache due to an anchor or scan budget.
...
Revert visibility, the revert dock, user-message history, and slash undo/redo use transcript `messageOrder` (conversation position), not id lexicographic comparison. A missing revert target fails visible rather than hiding by id. `revertToMessage` / slash redo do not materialize a refetch into the live timeline. Editing a sent message may await per-message `materializeTranscriptMessage` only when an authored file part is slim or missing `url`, then restore from the re-read parts.

`revertToMessage`, `unrevertSession`, `commitStagedRevertBeforeSend`, and the complete `commitMessageEdit` transaction share `session-history-mutation-coordinator.ts`, keyed by `[transportIdentity, generation, directory, sessionId]`. Same-session mutations run in call order; different sessions remain parallel. Queued operations re-read state after the queue wait and validate the captured runtime after awaits. Failure releases the lane. `stageMessageEdit` only restores a draft; its optional `{ directory, draftKey }` and opaque CAS `rollback()` handle retain the existing draft-ownership contract. Cancel/blur/navigation before submission never mutates remote history.

Primary edit submission keeps `messageEditCommitting` painted through interrupt → idle → native `revert.stage({ messageID, files: false })` → `revert.commit()`. It does not fetch a bounded membership snapshot or call the unavailable V1 `deleteMessage`. OpenCode owns the full forward boundary, including unloaded history. Both direct and queued replacement admission occur after commit; an already-admitted `preserveMessageId` / pending replacement fails closed because V2 cannot exclude it from truncation. The directory override routes the entire transaction to the session's own scope. Confirmed commit applies `revert-committed` to every current-runtime canonical scope, retiring stale reads and clearing durable tail copies.

Interrupt/idle/stage failures preserve the draft and transcript. An existing staged file rollback must be resolved first: upstream stage(files:false) would otherwise restore that operation's original snapshot and alter the working tree. Commit rejection clears the temporary revert with `DELETE /api/session/:sessionID/revert`, or restores a pre-existing conversation-only boundary. Cleanup failure keeps the last confirmed marker for recovery rather than pretending rollback succeeded. For ambiguous transport failures, an exact official message read returning the native `MessageNotFoundError` establishes that the boundary was removed; read failure leaves the outcome unresolved and blocks replacement send. A failed replacement after confirmed commit keeps the draft for ordinary resend. Runtime switches block subsequent requests, local publication, and replacement send to the new endpoint.

Query transcript repositories deduplicate HTTP/SSE commit acknowledgements using at most 256 receipts keyed by transport/generation/directory/session/boundary, purged with the generation or repository. This prevents a duplicate echo from invoking missing-boundary recovery and clearing a replacement in an incomplete history window. Conversation order, not message-ID rank, governs the store-adapter fallback too.
...
7. Edit staging restores the composer from the visible user-message snapshot captured at click time. Primary send interrupts a busy turn, waits for idle, and commits the native V2 history boundary with `files:false` before admitting the optimistic replacement. Pre-commit failures retain the draft; confirmed commit removes the old tail in conversation order. A replacement-send failure keeps the draft for ordinary retry. Leaving the session disarms `stagedMessageEdit`.
