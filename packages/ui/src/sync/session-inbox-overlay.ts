...
export const useSessionInboxOverlayStore = create<SessionInboxOverlayState>((set, get) => ({
  ...
  forgetPromoted(sessionID, promotedIDs) {
    ...
    set((state) => {
      const current = state.bySession[sessionID]
      if (!current?.length) return state
      // Authority-confirmed transcript identities retire queue and steer chips
      // independently of the running turn. Receipts fence late admission replies.
      const removable = new Set(current.filter((entry) => promoted.has(entry.id)).map((entry) => entry.id))
