/**
 * Cap ArchivedSessionsDialog + ArchivedSessionsManager for Lynx.
 * Dialog = SessionsSheet overlay. Page = Settings `archived-sessions` in-tab body.
 * Reuses listLynxArchivedSessions / unarchiveLynxSession — no duplicate API clients.
 */
...
export type LynxArchivedSessionsManagerMode = 'dialog' | 'page';

export type LynxArchivedSessionsManagerProps = {
  locale: string;
  mode?: LynxArchivedSessionsManagerMode;
  /** Dialog: sheet open. Page: always true. */
  active?: boolean;
  ...
  onMutated?: () => void;
  onPreview?: (sessionId: string, directory: string | null) => void;
};
...
/**
 * Cap ArchivedSessionsManager — project buckets → session list → restore/preview.
 * Settings page mode and SessionsSheet dialog share this body.
 */
export function LynxArchivedSessionsManager({
  locale,
  mode = 'dialog',
  active = true,
  indexState = null,
  ...
  onMutated,
  onPreview,
}: LynxArchivedSessionsManagerProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // Failure ≠ empty success — keep list empty only as a failed load, with error.
      setSessions([]);
  ...
  const handlePreview = (session: LynxArchivedSessionRow) => {
    (onPreview ?? onSelectSession)?.(session.id, session.directory);
  };

  const ListHost = mode === 'page' ? LynxView : LynxScrollView;
  const surfaceAttr = mode === 'page' ? 'page' : 'body';

  return (
    <LynxView
      data-lynx-archived-sessions={surfaceAttr}
    ...
    >
      ...
      <ListHost style={{ flexGrow: 1, minHeight: '0' }}>
        {loading ? (
...
export function LynxArchivedSessionsDialog({
  ...
  const handlePreview = (sessionId: string, directory: string | null) => {
  ...
  return (
    <LynxMobileResizableSheet
      locale={locale}
      open={open}
      title={lynxT(locale, 'lynx.chat.archived.title')}
