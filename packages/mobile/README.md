...
- Shared UI (`packages/ui/src/lib/native-ios-tab-bar.ts`, `useNativeIosTabBar`) presents the overlay from `MobileTabsRoot` on Capacitor iOS when `openchamber.iosNativeUi` is on. Labels come from existing `mobile.tabs.*` keys. Each tab may carry an optional `badge` display string (`string | null`); UI owns formatting (including the `99+` cap), and native only maps non-empty strings onto `UITabBarItem.badgeValue` (null/empty clears). Badge-only updates reuse the installed items instead of rebuilding chrome. The overlay hosts a chrome-only `UITabBarController` (clear placeholder pages, pass-through hit testing) so iOS 26 owns the floating liquid-glass capsule, the selected liquid-lens indicator, and light/dark materials (`overrideUserInterfaceStyle` follows the Web theme). The selected glyph uses the theme `--primary` tint; unselected items use `secondaryLabel`. Custom nested glass pills and hover lift/highlight are not used. `:root.oc-native-ios-tab-bar` marks adoption; the Web dock unmounts once adopted.
...
- `OpenChamberLiveActivity` is an iOS-only Capacitor plugin. `canUseNativeIosLiveActivity` is Capacitor iOS + plugin registration only — not gated by `openchamber.iosNativeUi`. Runtime support is **iOS 17.0+** (the `OpenChamberWidget` deployment target) and `ActivityAuthorizationInfo.areActivitiesEnabled`. The App target stays at **15.5** and compiles behind `canImport(ActivityKit)` + `@available(iOS 17.0, *)`. `isSupported` / `start` / `update` / `end` all use that same floor; `update` and `end` below it reject as unsupported instead of succeeding silently.
- One Live Activity for all live working sessions, not just the currently selected top-level session. JS starts it after work stays busy for 5 seconds. The lock screen and expanded Dynamic Island show `{n} working` plus a row per session (title, checkmark or spinner, elapsed `{n}m`). Each row is a `Link` to that session. `Activity.request` uses `pushType: .token` so APNs can update that existing Live Activity (token available since iOS 16.1; this is not 17.2 push-to-start). The push token belongs to that Activity instance, not the app-wide device token; native code forwards `{activityId, sessionId, token}` on `pushToken`, including rotations.
- While the App is active, semantic updates apply immediately through the local ActivityKit bridge. When it is not, APNs remote updates the existing Live Activity, and the server compensates terminal complete/error states if the process never resumes.
- Timing is ActivityKit's: `staleDate` is `updatedAt + 20 minutes`. In-progress elapsed time uses the system auto-updating timer and freezes at `endedAt` on complete/error. `end` must pass final content; dismissal defaults to 15 minutes on success and 60 minutes on error (`dismissalSeconds` may override).
