...
다른 사람이 인스턴스에 접근할 수 있다면, 특히 공용 인터넷이나 리버스 프록시를 통해 접근할 수 있다면 항상 비밀번호를 설정하세요.
...
- 공개 URL이 필요하다면 인터넷에 포트를 여는 것보다 이나 사설 네트워크(예: VPN)를 선호하세요.
- OpenChamber를 자체 HTTPS 서버 뒤에 둔다면 [Reverse Proxy](/ko/reverse-proxy/)를 참고하세요.

## 관련 항목

- [기기 연결](/ko/connect-devices/) — 일회용 페어링과 기기별 토큰
- [Private Relay](/ko/private-relay/) — 어디서든 종단 간 암호화 접속
- [리버스 프록시](/ko/reverse-proxy/) — 자체 서버 뒤에서 OpenChamber 실행
