...
import {
  ...
  projectSessionMessageRecords as projectRecords,
...
const createV2FetchPage = ({ buildOpenCodeUrl, getOpenCodeAuthHeaders, logger }) => {
  return async ({ sessionID, before, limit, signal }) => {
    ...
    // Upstream rejects `order` together with `cursor` (400); the cursor
    // already encodes the desc direction of the first page.
    const result = await client.message.list({
      sessionID,
      ...(Number.isFinite(limit) ? { limit } : {}),
      ...(typeof before === 'string' && before.length > 0 ? { cursor: before } : { order: 'desc' }),
    }, { signal });
...
const createSdkFetchMessage = ({ buildOpenCodeUrl, getOpenCodeAuthHeaders, logger }) => {
  return async ({ sessionID, messageID, signal }) => {
    ...
    const projected = projectSessionMessage(data);
    // Control rows (idle / model-switched) are not chat messages.
    if (!projected || !projected.info) {
      const notFound = new Error('not_found');
      notFound.code = 'not_found';
      throw notFound;
    }
