...
export const createMessageQueueService = ({ dbPath, getRuntimeConfig = () => null, clock = () => Date.now(), isServerPathAllowed = () => false, onRevisionTip = null } = {}) => {
  ...
  const validateComposer = (document, content) => {
    ...
    for (const reference of document.references) {
      ...
      } else if (reference.kind === 'skill' && Object.keys(reference).every((key) => ['id', 'kind', 'start', 'end', 'display', 'skillName'].includes(key)) && nonEmptyString(reference.skillName, MAX_COMPOSER_SKILL_NAME) && /^[^\s/\\[\]\r\n]+$/u.test(reference.skillName) && [`/${reference.skillName}`, `/\u2003${reference.skillName}`, `@${reference.skillName}`, `@\u2003${reference.skillName}`].includes(reference.display)) canonical += `[skill:${reference.skillName}]`;
