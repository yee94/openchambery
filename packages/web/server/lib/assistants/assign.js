...
const promptAdmitted = (result) => (
  !result?.error
  && (result?.response?.status === 204 || result?.status === 204 || result?.id != null || result?.type != null || result?.data !== undefined || result?.response?.ok === true)
);
...
export async function assignSession(input = {}) {
  ...
  const created = await input.createSession({ directory, title, metadata, agent: trim(assistant.agent, 256), model: { id: modelID, providerID, ...(trim(assistant.variant, 256) ? { variant: trim(assistant.variant, 256) } : {}) } });
