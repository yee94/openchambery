...
const createV2Client = (overrides = {}) => {
  ...
  const merged = { ...sessionClient, ...sessionOverrides };
  const create = merged.create ?? (async () => ({ id: crypto.randomUUID() }));
  const get = merged.get ?? (async () => ({ id: 'present' }));
  const prompt = merged.prompt ?? merged.promptAsync ?? (async () => ({ id: 'inbox_1', type: 'user' }));
  const compact = merged.compact ?? merged.summarize ?? (async () => ({ id: 'inbox_compact', type: 'compaction' }));
  const interrupt = merged.interrupt ?? merged.abort ?? (async () => {});
  const rename = merged.rename ?? (async () => {});
  const remove = merged.remove ?? merged.delete ?? (async () => {});
  const update = merged.update ?? (async () => ({ id: 'archived' }));
  const list = merged.messages ?? merged.list ?? (async () => ({ data: [], cursor: {} }));
  const switchAgent = merged.switchAgent ?? (async () => {});
  const switchModel = merged.switchModel ?? (async () => {});
  const putInstruction = merged.putInstruction ?? (async () => {});
  const active = merged.active ?? (async () => ({}));
  // Exact message lookup for assign ambiguous admission (overrides.message).
  const sessionMessage = typeof merged.message === 'function' ? merged.message : null;
  const modelList = overrides.model?.list
    ?? (async () => ({ data: [] }));
  ...
  return {
    ...
    provider: { list: providerList, ...(provider || {}) },
    model: { list: modelList, ...(overrides.model || {}) },
    ...(config ? { config } : {}),
  };
};
...
describe('assistants service', () => {
  ...
  it('admits 33-part direct messages and 129-part shares', async () => {
    ...
    // Composer send is the contact harness (no session.prompt). Share + queued
    // delivery use v2 session.prompt (toV2PromptInput joins text parts).
    expect(prompts).toHaveLength(2);
    expect(prompts.every((prompt) => typeof prompt.text === 'string' && prompt.text.split('\n').length === 129)).toBe(true);
    service.close();
  });
  ...
  it('applies the complete workspace patch and creates metadata with the final name', async () => {
    ...
    const updated = await service.updateAssistant(assistant.id, { expectedRevision: 1, workspacePath: other, name: 'Renamed', defaultPrompt: 'P', providerID: 'provider-2', modelID: 'model-2', agent: 'agent-2', enabled: false }); expect(updated).toMatchObject({ name: 'Renamed', defaultPrompt: 'P', providerID: 'provider-2', modelID: 'model-2', agent: 'agent-2', enabled: false, sessionID: 'ses_workspace' }); expect(updated).not.toHaveProperty('skillRoots');
    // v2 create uses location.directory + title prefix; archive metadata write is unavailable.
    expect(created).toMatchObject({ title: '[Assistant] Renamed', location: { directory: fs.realpathSync(other) } });
    service.close();
  });

  it.skip('creates Assistant sessions with a fixed title prefix, ownership metadata, and archive-before-bind', async () => {
    const directory = root();
    const order = [];
    let created;
    let archived;
  ...
  it.skip('does not bind or prompt when archive fails after create', async () => {
    let creates = 0;
    let prompts = 0;
    const service = setup(root(), {
      create: async () => ({ data: { id: `ses_${++creates}` } }),
  ...
  it.skip('retries archive once on 404 then binds after success', async () => {
    let updates = 0;
    const service = setup(root(), {
  ...
  it('persists nullable variants and sends the captured OpenCode variant for messages and shares', async () => {
    ...
    const models = [];
    const service = setup(root(), {
      promptAsync: async (input) => { prompts.push(input); return { response: { status: 204 } }; },
      switchModel: async (input) => { models.push(input); },
    });
    ...
    // Composer send is the contact harness. Share still applies the OpenCode variant via switchModel.
    expect(models).toEqual(expect.arrayContaining([expect.objectContaining({ model: expect.objectContaining({ variant: 'fast' }) })]));
    expect(prompts.some((prompt) => prompt.text === 'share')).toBe(true);
  ...
  it('uses the workspace directory when submitting shares', async () => {
    ...
    await service.share(assistant.id, { operationID: 'share_directory', payload: { messageID: 'client_share_directory', parts: [{ type: 'text', text: 'shared' }] } });
    // v2 prompt carries sessionID + text; directory is binding-scoped, not a prompt field.
    expect(prompt.sessionID).toBeTruthy();
    expect(prompt.text).toBe('shared');
    expect(service.snapshot().assistants[0].effectiveWorkspacePath).toBe(fs.realpathSync(workspace));
    service.close();
  });
  ...
  it('invalid explicit worker model fails closed with no session create', async () => {
    ...
    const service = setup(directory, {
      ...
      promptAsync: async () => ({ response: { status: 204 } }),
      provider: {
        list: async () => ({ data: [{ id: 'xai', name: 'xAI' }] }),
      },
      model: {
        list: async () => ({ data: [
          { providerID: 'xai', id: 'grok-4.6', name: 'Grok 4.6', capabilities: { input: { image: true } } },
        ] }),
      },
    }, {
      runContactTurn: async ({ tools }) => {
  ...
  const sessionFollowCatalog = {
    ...
    model: {
      list: async () => ({ data: [
        { providerID: 'xai', id: 'grok-4.6', name: 'Grok 4.6', capabilities: { input: { image: true } } },
        { providerID: 'openai', id: 'gpt-4o', name: 'GPT-4o', capabilities: { input: { image: true } } },
        { providerID: 'p', id: 'm', name: 'M' },
      ] }),
    },
  };
...
describe('contact current-instance model context', () => {
  const catalogClient = (id) => ({
    provider: { list: async () => ({ data: [{ id, name: `Provider ${id}` }] }) },
    model: { list: async () => ({ data: [
      { providerID: id, id: 'model', name: `Model ${id}` },
      { providerID: 'disconnected', id: 'hidden', name: 'Hidden' },
    ] }) },
  });
  ...
  it('distinguishes failed catalog lookup from a successful empty instance', async () => {
    ...
    let failed = true;
    const service = setup(root(), {
      provider: { list: async () => { if (failed) throw new Error('unavailable'); return { data: [] }; } },
      model: { list: async () => { if (failed) throw new Error('unavailable'); return { data: [] }; } },
    }, {
...
describe('read_session referenced conversation', () => {
  ...
  it('uses OpenCode.make session/message methods and does not call 1.x SDK names', async () => {
    ...
    // Abort may short-circuit on contact-turn controllers; when it reaches OpenCode it uses interrupt.
